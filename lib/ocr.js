...
export function extractDocumentData(text) {
  ...
  const keywords = {
    ...
    'Tax': ['TAX RECEIPT', 'ROAD TAX', 'MV TAX', 'VEHICLE TAX']
  };
  ...
  // 4. Dependency Logic
  if (detectedType === 'RC' || detectedType === 'Gujarat Permit') {
    const dependencyKeywords = ['AS PER FITNESS', 'SUBJECT TO FITNESS', 'VALID WITH FC', 'CO-TERMINUS WITH FITNESS', 'CO TERMINUS WITH FITNESS'];
    if (dependencyKeywords.some(k => rawText.includes(k))) {
      expiryType = 'FITNESS_LINKED';
      expiryDate = null; // Direct date is invalid if fitness linked
    }
  } else if (detectedType === 'National Permit') {
